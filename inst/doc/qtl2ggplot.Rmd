...
dirpath <- "~/Documents/Talks/PlantSysGen/data/DO_mice"
...
```{r eval=FALSE}
coefs2 <- doqtl2::listof_scan1coefCC(as.data.frame(DOex$pheno), covar=NULL, probs=apr, K=NULL)
```
...
doqtl2:::plot.listof_scan1coefCC(coefs2, out_allele)
```
...
```{r}
coefs3 <- coefs2[[1]]
coefs3$coef <- rbind(coefs3$coef, coefs2[[2]]$coef)
```

```{r}
phe_names <- names(coefs2)
plot_coefCC(coefs3, 
            pattern = rep(phe_names, each = nrow(coefs3$coef) / length(phe_names)),
            patterns = "all", facet = "pattern")
```
...
- facet = "pattern" does not work properly; acts wierdly
- want to make sure patterns are in order based on LOD

```{r}
plot_snpasso(out_snps,1:2, facet="pheno", cex=1, pch=1)
```
...
```{r}
plot_snpasso(out_snps,1:2,show_all_snps = FALSE, facet_var = "pheno",
...
```{r}
plot_snpasso(out_snps, 1:2, patterns = "all", cex=2, pch=1,
             facet_var = "pheno", drop.hilit=3)
```

```{r}
plot_snpasso(out_snps, 1:2, patterns = "hilit", cex=2, pch=1,
             drop.hilit=3, ylim=c(3.6,6.6), facet_var = "pheno")
```

```{r}
plot_snpasso(out_snps, 1:2, patterns = "hilit", show_all_snps = TRUE, cex=2, pch=1,
