...
```{r}
autoplot(scan_snppr, snpinfo, facet = "pheno", drop.hilit = 1.5)
```
...
```{r}
autoplot(scan_snppr, snpinfo, show_all_snps=FALSE, drop.hilit = 1.5)
```
...
Plot results. Note that `plot.scan1` is not passing lodcolumn to `plot_snpasso` so temporary workaround.

```{r}
plot(scan_snppr, snpinfo, lodcolumn=1, cex=1, pch=1, drop_hilit = 1.5)
plot_snpasso(scan_snppr, snpinfo, lodcolumn=2, cex=1, pch=1, drop_hilit = 1.5)
```
...
plot_snpasso(scan_snppr, snpinfo, lodcolumn=2, cex=1, pch=1, 
             show_all_snps = FALSE, drop_hilit = 1.5)
```

```{r}
autoplot(scan_snppr, snpinfo, 1:2, show_all_snps = FALSE, facet="pheno", cex=2, pch=1, drop.hilit = 1.5)
```
