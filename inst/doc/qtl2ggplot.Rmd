...
```{r}
summary(coefs, out_allele, DOex$pmap)
```
...
```{r}
summary(out_snps, DOex$pmap, snpinfo)
```
...
```{r}
top_snps_tbl <- qtl2pattern::top_snps_all(out_snps, snpinfo)
```
